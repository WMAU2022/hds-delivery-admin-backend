...
console.log(`🕐 Timezone: Sydney (Australia/Sydney)\n`);

// Sydney Metro schedule
const sydneySchedule = {
...
console.log('🚚 SYDNEY METRO SCHEDULE:\n');

// Collect all next delivery dates
const results = [];
for (const [deliveryDay, config] of Object.entries(sydneySchedule)) {
...
// Print results
console.log('Schedule\t| Cutoff\t\t| Next Available Delivery');
console.log('-------\t\t| -------\t\t| ----');
for (const r of results) {
  const cutoffStr = `${r.cutoffDay} ${r.cutoffTime}`;
  console.log(`${r.deliveryDay}\t\t| ${cutoffStr.padEnd(16)} | ${r.dayName} ${r.formatted}`);
}
...
for (const r of results) {
  ...
  console.log(`  - Days from now (Tue Aug 11) to cutoff: ${dayNames.indexOf(r.cutoffDay)} - ${testDate.getDay()} = ...`);
